...
const Player = ({
  ...
  const onNextOrPrev = index => {
    ...
    setCurrentSong({
      name: songList[songId].name,
      uri: songList[songId].fileCopyUri,
      index: songId,
    });
  };
  ...
  const changeValue = e => {
    onSkip(
      (((e.nativeEvent.pageX * 100) / (screenWidth - 10)) * duration) / 100,
    );
  };
        ...
        <StyledView
          ...
          {duration && (
            <TouchableWithoutFeedback onPress={changeValue}>
              <Progress.Bar progress={time} width={null} height={10} animated />
          ...
          <StyledView flexDirection={'row'} justifyContent={'space-around'}>
            ...
            <PlayerButton
              Icon={ICSkipLeft}
              onPress={() => {
                onNextOrPrev(currentSong.index - 1);
              }}
            />
            <PlayerButton
              Icon={isPlay ? ICPause : ICPlay}
