...
const FingerPrintScannerPage = () => {
  ...
  const startScan = () => {
    FingerprintScanner
    .authenticate({ description: I18nJs.t('scanText') })
    .then(() => {
      setIsErr(false)
      hideInfoMessage()
    })
  ...
  const hideInfoMessage = (err) => {
    let timer = 2000
    if(err){
     timer = JSON.stringify(err).includes('AuthenticationFailed') ? 60000 : 2000
    }
    let interval
    ...
    >
      {isErr !== null && (
        <InfoMessage
          ...
          text={isErr ? `${I18nJs.t('scanErr')} ${timerCount}s` : I18nJs.t('scanSuccess')}
        />
      )}
      <CustomButton
        text={I18nJs.t('fingerScannerButton')}
        bodyStyles={{backgroundColor: 'lightblue', marginTop: 10, borderRadius: 20, alignSelf: 'center'}}
